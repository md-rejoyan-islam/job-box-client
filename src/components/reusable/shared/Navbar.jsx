...
const Navbar = () => {
  ...
  return (
    <nav className="h-16 px-4 fixed w-full  z-[100] bg-white/65 ">
      {/* horizontal menu  */}
      ...
      <ul className="  flex sm:hidden max-container  gap-x-6 h-full items-center relative ">
        ...
        <div
          ...
          } z-100 fixed top-0  h-full  transition-transform duration-700   border-none     flex  justify-end  w-full left-0`}
        >
          <div
      ...
      <div
        className={`${
          isOpen ? "" : "hidden"
        }bg-black/50 sm:hidden w-full h-screen -z-10  absolute top-0 left-0 `}
      ></div>
    </nav>
  );
};
